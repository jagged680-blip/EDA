...
// 1. ИНИЦИАЛИЗАЦИЯ И СЕТКА
function drawGrid(layer) {
    const group = new Konva.Group({ listening: false });
    const size = 4000;
    for (let i = 0; i <= size / GRID_SIZE; i++) {
        group.add(new Konva.Line({ points: [i * GRID_SIZE, 0, i * GRID_SIZE, size], stroke: '#222', strokeWidth: 1 }));
        group.add(new Konva.Line({ points: [0, i * GRID_SIZE, size, i * GRID_SIZE], stroke: '#222', strokeWidth: 1 }));
    }
    layer.add(group);
    group.moveToBottom();
}
...
function connectPins(sourcePin, targetPin) {
    const wireLine = new Konva.Line({ stroke: '#00ff00', strokeWidth: 2, lineCap: 'round', points: [0,0,0,0] });
    const wireObj = { id: 'wire_' + Math.random().toString(36).substr(2, 5), source: sourcePin, target: targetPin, line: wireLine };
    wires.push(wireObj);
...
window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Delete' || e.key === 'Backspace') {
        wires = wires.filter(w => {
            ...
            if ((sComp && sComp.id() === selectedId) || (tComp && tComp.id() === selectedId)) {
...
function addComp(prefix, color, w_notused, h_notused, typeName, manualId = null) {
    ...
    else { counters[typeName]++; id = prefix + counters[typeName]; }

    const groupSch = new Konva.Group({ 
        x: lastPointerPos.x, y: lastPointerPos.y, 
        draggable: true, id: id, name: 'comp-group',
        dragBoundFunc: pos => ({ x: Math.round(pos.x/GRID_SIZE)*GRID_SIZE, y: Math.round(pos.y/GRID_SIZE)*GRID_SIZE })
    });

    const createPin = (px, py) => {
        const p = new Konva.Circle({ x: px, y: py, radius: 4, fill: 'yellow', name: 'pin', stroke: '#555', strokeWidth: 1, hitStrokeWidth: 15 });
        p.on('mousedown', (e) => {
            e.cancelBubble = true;
            if (!connectionSource) {
    ...
    if (typeName === 'Resistor') {
        ...
        for(let i=0; i<3; i++) {
            groupSch.add(new Konva.Arc({ name:'comp-body', x:-15+(i*15), y:0, innerRadius:7, outerRadius:7, angle:180, rotation:180, stroke:'white', strokeWidth:1.5 }));
        }
    ...
    const groupPcb = new Konva.Group({
        x: 60, y: 60, draggable: true, id: id,
        dragBoundFunc: pos => ({ x: Math.round(pos.x/GRID_SIZE)*GRID_SIZE, y: Math.round(pos.y/GRID_SIZE)*GRID_SIZE })
    });
...
function saveProject() {
    ...
    wires.forEach(w => {
        data.wires.push({
            sourceId: w.source.findAncestor('.comp-group').id(),
            sourcePinIdx: w.source.getParent().getChildren().filter(n => n.name() === 'pin').indexOf(w.source),
            targetId: w.target.findAncestor('.comp-group').id(),
            targetPinIdx: w.target.getParent().getChildren().filter(n => n.name() === 'pin').indexOf(w.target)
        });
    });
...
function loadProject(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            wires.forEach(w => w.line.destroy()); wires = [];
            ...
            data.components.forEach(c => {
                ...
                const g = stageSch.findOne('#' + c.id);
                if(g) g.rotation(c.rotation || 0);
            });
            layerSch.batchDraw();
            setTimeout(() => {
                if (data.wires) {
                    data.wires.forEach(w => {
                        const sComp = stageSch.findOne('#' + w.sourceId);
                        const tComp = stageSch.findOne('#' + w.targetId);
                        if (sComp && tComp) {
                            const sPin = sComp.find('.pin')[w.sourcePinIdx];
                            const tPin = tComp.find('.pin')[w.targetPinIdx];
                            if (sPin && tPin) connectPins(sPin, tPin);
                        }
                    });
                }
                updateWires(); layerSch.draw(); layerPcb.draw();
            }, 50);
            event.target.value = '';
